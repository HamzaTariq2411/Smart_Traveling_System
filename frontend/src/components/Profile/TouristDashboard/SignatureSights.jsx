  ...
  return (
    <div className="fixed inset-0 text-black bg-black bg-opacity-50 flex items-center justify-center">
      <div className="bg-white p-6 rounded-lg shadow-lg">
        ...
        <p className="mb-4">
          Do you allow us to access your location to show the nearest tourist
          attractions?
        </p>
...
const SignatureSights = () => {
  ...
  const [isViewingNearest, setIsViewingNearest] = useState(false); // State to track if user is viewing nearest attractions
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          toast.dismiss();
          toast.success("Now you are seeing the nearest tourist attractions");
        },
        (error) => {
  ...
  const handleShowNearestPlaces = () => {
    if (isViewingNearest) {
      ...
      toast.dismiss();
      toast.success("Now you are seeing all tourist attractions");
    } else {
      setIsModalOpen(true);
    }
  };
    ...
    <div className="ml-14 px-4 py-8">
      ...
      <div className="mb-4 flex gap-3 md:flex-row flex-col">
        ...
        <button
          ...
          className=" px-4 h-10 md:w-1/5 w-auto text-sm bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
          {isViewingNearest ? "Show all" : "Show nearest attractions"}
        </button>
      </div>
      <Modal
        isOpen={isModalOpen}
      ...
      <ul className="flex flex-wrap justify-center gap-6">
        {filteredData?.slice(0, visibleItems)?.map((place) => (
          <li
            ...
            className="bg-white md:w-2/5 w-11/12 my-2 sm:p-3 p-2 rounded-lg shadow-md"
          >
            <div className="flex justify-center mb-4">
              {place?.image_urls && (
                <img
                  src={place?.image_urls}
                  alt={place?.name}
            ...
            <p className="text-gray-600 mb-2 text-xs">{place?.description}</p>
...
export default SignatureSights;
